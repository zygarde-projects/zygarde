...
import zygarde.codegen.model.WebMvcApiGenerateResult
...
class WebMvcApiGenerator(
  ...
  fun generateApis(): WebMvcApiGenerateResult {
    ...
    return WebMvcApiGenerateResult(
      ...
      webMvcControllers = webMcvControllerFileSpecBuilderMap.values.map { it.build() },
      serviceInterfaces = serviceInterfaceFileSpecBuilderMap.values.map { it.build() },
    )
  }

  private fun ApiVo.generate() {
    ...
    val webMvcControllerBuilder = webMcvControllerBuilderMap.getOrPut(apiImplName) {
      TypeSpec.classBuilder(apiImplName)
        ...
        .also { typeSpec ->
          basePath?.let {
            typeSpec.addAnnotation(AnnotationSpec.builder(RequestMapping::class).addMember("value=%S", it).build())
          }
        }
        .addAnnotation(
      ...
      val methodAnnotation = AnnotationSpec
        ...
        .also { annSpec ->
          if (func.path.isNotEmpty()) {
            annSpec.addMember("value=[%S]", func.path)
          }
        }
        .build()
