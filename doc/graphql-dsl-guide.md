...
operation 函式與型別定義都可以加上 SDL description,會在 schema 產出物寫成 GraphQL 的 block string(`"""..."""`),於 introspection / GraphiQL 顯示。
...
目前 description 只支援 operation 函式與型別定義層級;個別 field / argument / enum value 的 description 尚未支援。
...
- **description** — operation 函式與型別定義的 `description` 可省略;一旦設定就不可為空白字串。
...
DSL 目前涵蓋 query / mutation / subscription、參數、型別定義、預設值與 operation/型別層級的 description。以下尚未由 DSL 支援,需要時請手寫 resolver:

- **field / argument / enum value 的 description** — 目前 description 只支援 operation 函式與型別定義層級。
